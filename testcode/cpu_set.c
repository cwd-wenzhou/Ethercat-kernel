...
#define CPU 2
#define cyctime 200000 //ns
#define TASK_FREQUENCY 2000        //*Hz* 任务周期
...
struct MOTOR {
    ...
    int32_t targetVelocity;   //电机的目标速递
...
void cyclic_task(){
    ...
    while (running){
        ...
        //读取数据
        motor.status = EC_READ_U16(motor.domain_pd + motor.drive_variables.status_word);  //状态字
        motor.opmode = EC_READ_U8(motor.domain_pd + motor.drive_variables.mode_display);  //运行模式
        motor.currentVelocity = EC_READ_S32(motor.domain_pd + motor.drive_variables.current_velocity);  //当前速度
        motor.currentPosition = EC_READ_S32(motor.domain_pd + motor.drive_variables.current_postion);  //当前位置
        ...
        if(motor.driveState == dsOperationEnabled && motor.resetBusy == 0 &&
            ...
            if(motor.opmode == 8) {  //速度模式
                if(motor.homeBusy == true) {  //开始回零
                    ...
                    //printf("黄河电机实时位置%d\t,电机目标位置%d\n",motor.currentPosition,motor.targetPosition);
                    if(motor.positionMoving == true) {  //开始运动
                        if(position_count == 8000) {
                            position_count = 0;
                            t = 0;
                        }
                        motor.targetPosition = (int32_t)(ENCODER_RESOLUTION * sin(Pi * t) / 2);  //位置模式时传送位置信息
                        //printf("电机实时位置%d\t,电机目标位置%d\n",motor.currentPosition,motor.targetPosition);
                        EC_WRITE_S32(motor.domain_pd + motor.drive_variables.target_postion, motor.targetPosition);
                        //写入地址空间end
...
{ 
    ...
    signal(SIGINT,close_sig_handler);//ctrl+c的函数设置

    motor.targetPosition = 0;
    motor.opModeSet = 8;         //速度模式
    motor.powerBusy=1;
    //motor.homeBusy =1;
    //motor.targetVelocity=ENCODER_RESOLUTION/60;
    motor.positionMoving = 1;
    /* Get current time and compute the next nanosleeptime */
    
    clock_gettime(CLOCK_REALTIME, &nextnanosleeptime);
    nextnanosleeptime.tv_nsec += 1000000;
    nanoSecondFieldConversion(&nextnanosleeptime);
    //int8_t reset_count = 0;      //复位的时候计数用
    //int16_t position_count = 0;  //位置模式下计数
    //double t = 0.00;
